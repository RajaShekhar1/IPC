...
Feature: TAA should allow only certain combinations of riders on some base products.
  ...
  Scenario: Ensure QOL3 rider is not allowed on FPP-Gov if WP is configured.
    Given I am setting up a case enrolling the 'FPP-Gov' base product
    When I check the following riders for compatibility
      | AIR | WP | QOL3 | QOL4 |
      | N   | Y  | Y    | N    |
    Then It should show that the riders 'QOL3' and 'WP' cannot be combined.

  Scenario: Ensure QOL4 rider is not allowed on FPP-Gov if WP is configured.
    Given I am setting up a case enrolling the 'FPP-Gov' base product
    When I check the following riders for compatibility
      | AIR | WP | QOL3 | QOL4 |
      | N   | Y  | N    | Y    |
    Then It should show that the riders 'QOL4' and 'WP' cannot be combined.
