var CaseSettingsPanel = function(case_data, product_choices, can_edit_case, settings) {
  var self = this;
  ...
  self.emailSettings = {
    ...
    subject: ko.observable(case_data.self_enrollment_setup.email_subject),
    ...
    message: ko.observable(case_data.self_enrollment_setup.email_message)
  };
  ...
  self.landing = {
    page_title: ko.observable(case_data.self_enrollment_setup.page_title),
    page_text: ko.observable(case_data.self_enrollment_setup.page_text),
  };
  ...
  _.each(self.annual_enrollment_periods(), function(period) {
    _.each([period.start_date, period.end_date], function(date_observable){
      date_observable.subscribe(function(val) {
        ...
        var val_date = parse_date(val, "MM/DD");
        if (!val_date.isValid()) {
          date_observable("");
  ...
  if (self.can_edit_case) {
    ...
    // For now, trigger dirty on every change within the self-enroll setup.
  ...
  self.check_unique_name = function(current_value, callback) {
    $.get(urls.get_cases_api_url(), {by_name: current_value}, function(result) {
  ...
  self.remote_delete = function() {
    ...
    $.when(request).then(function() {
      ...
      window.location.href = urls.get_manage_case_url(case_data.id);
    }, function() {
  ...
  self.sammy_app = Sammy(function() {
    ...
    this.get("#enrollment", function() {
      ...
      function handle_census_data_loaded(table, data) {
        window.case_settings.census_data(data);
        window.case_settings.toggle_enrollment_buttons();
      }

      function handle_no_census_data_loaded() {
        window.case_settings.census_data([]);
        window.case_settings.toggle_enrollment_buttons();
      }
    });
