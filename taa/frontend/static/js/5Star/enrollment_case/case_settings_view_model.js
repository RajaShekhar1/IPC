var CaseSettingsPanel = function CaseSettingsPanel(case_data, product_choices, can_edit_case, settings) {
  ...
  self.partner_agents = ko.observable(
    ...
    $.getJSON(urls.get_case_api_census_email_batches(window.case_id), function(data) {
      self.email_batches(data.data);
    });

    self.batch_preview = function(batch) {
      window.open(urls.get_case_api_census_email_batch_preview_url(window.case_id, batch.id), "batchView", "menubar=no,location=no,resizable=no,width=650,height=600,scrollbars=yes,status=yes");
    };

    self.load_logs = function(batch) {
      $("#log"+batch.id).html("Loading Logs...");
      $.get(urls.get_case_api_census_email_batch_logs_url(window.case_id, batch.id), function(data) {
    ...
    self.get_form_error = ko.computed(function() {
      ...
      self.check_unique_name = function(current_value, callback) {
        $.get(urls.get_cases_api_url(window.case_id), {by_name: current_value}, function(result) {
      ...
      self.serialize_enrollment_periods = function() {
        ...
        this.get("#history", function() {
          ...
          $.getJSON(urls.get_case_api_census_email_batches(window.case_id), function(data) {
            window.case_settings.email_batches(data.data);
          });
        });
