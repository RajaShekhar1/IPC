...
// AFBA custom max length limiter on text input fields
var maxlen_lookup = {
  'SSN': 9,
  'MBR-FNAME': 20,
  'MBR-LNAME': 28,
...
ko.bindingHandlers.maxlength = {
  init: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
    var attr_key = valueAccessor();
    var max_length = maxlen_lookup[attr_key];

    if (!max_length) {
      throw 'Invalid maxlength key '+ attr_key;
    }

    ko.bindingHandlers.attr.update(element, function() {return {maxlength: ''+max_length}}, allBindings, viewModel, bindingContext);
  }
};