...
return self.child_benefits().all_options();}); self.is_show_rates_clicked=ko.observable(false);self.is_employee_info_valid=ko.computed(function(){return self.insurance_product.is_valid_employee_age(self.employee().get_age());});self.is_spouse_age_valid=ko.computed(function(){return self.insurance_product.is_valid_spouse_age(self.spouse().get_age());});self.are_children_ages_valid=ko.computed(function(){var all_valid=true;$.each(self.children(),function(){if(!self.insurance_product.is_valid_child_age(this.get_age())){all_valid=false;return false;}});return all_valid;});self.can_display_rates_table=ko.computed(function(){
 
var valid=self.employee().is_valid();valid&=self.is_employee_info_valid(); 
