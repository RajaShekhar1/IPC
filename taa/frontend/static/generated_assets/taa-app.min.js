...
function show_loading_panel(){$(".form-panel").hide("slide");$(".loading-panel").show("slide");$(".processing-buttons").show("fade");$(".form-buttons").hide("fade");}
function show_error_panel(){$(".loading-panel").hide("slide");$(".error-panel").show("slide");$(".error-buttons").show("fade");$(".processing-buttons").hide("fade");}
function show_success_panel(){$(".loading-panel").hide("slide");$(".success-panel").show("slide");$(".success-buttons").show("fade");$(".processing-buttons").hide("fade");}
function show_form_panel(){$(".modal-panel").hide("slide");$(".buttons-panel").hide("slide");$(".form-panel").show("slide");$(".form-buttons").show("slide");}
function reset_upload_modal(){$(".modal-panel").hide();$(".buttons-panel").hide();$(".form-panel").show();$(".form-buttons").show();}
function handle_upload_success(resp){census_errors_panel.error_records(resp.data.errors);census_errors_panel.num_data_records(resp.data.records.length);show_success_panel();window.case_settings.census_data(resp.data.records);case_management.refresh_census_table(window.case_id,urls.get_case_api_census_records_url(window.case_id),"#census-records-table","#census-table-loading");}
function handle_upload_error(xhr){if(xhr.status>=400&&xhr.status<500){show_error_panel();var data=$.parseJSON(xhr.responseText).data;census_errors_panel.error_records(data.errors);$('.upload-error-table td.error').tooltip({placement:"auto left"});}else{alert("Sorry, there was a problem communicating with the server.");reset_upload_modal();}}
function observe_settings_events(settings){$("#upload-btn").on("click",reset_upload_modal);$(".back-btn").on("click",show_form_panel);$('#reset-page-text').click(function(){$('#page_text').html(settings.default_page_text).focus();});$('#self_enrollment_type').change(function(){if($('#self_enrollment_type option:selected').val()=='case-generic'){$('.self-enroll-email').hide();$('.self-enroll-email-off').show();}else{$('.self-enroll-email').show();$('.self-enroll-email-off').hide();}}).change();$('#edit-self-enrollment-form').on('submit',function(){window.case_settings.save_settings();return false;});}
...
form_data.append('csv-file',files[0],files[0].name);form_data.append('upload_type',$("input[name=upload_type]:checked").val());send_file_data("POST",urls.get_case_api_census_records_url(window.case_id),form_data,handle_upload_success,handle_upload_error,false);show_loading_panel();return false;});}
