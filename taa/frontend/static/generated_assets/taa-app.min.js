...
function observe_enrollment_upload_form_submit(){$("#enrollment-csv-form").on("submit",function(){var form=this;var form_data=new FormData();file_extension=files[0].name.split(".").slice(-1)[0];if(window.case_settings){form_data.append('case_token',window.case_settings.case_token);}
...
GroupCIProduct.prototype=Object.create(Product.prototype);GroupCIProduct.prototype.is_valid_employee=function(employee){var age=employee.get_age();var valid=(age>=this.min_emp_age()&&age<=this.max_emp_age());valid&=employee.is_smoker()!=null;valid&=employee.has_valid_weight();valid&=employee.has_valid_height();valid&=employee.has_valid_gender();return valid;};GroupCIProduct.prototype.is_valid_spouse=function(spouse){var age=spouse.get_age();var valid=(age>=this.min_sp_age()&&age<=this.max_sp_age());valid&=spouse.is_smoker()!=null;valid&=spouse.has_valid_weight();valid&=spouse.has_valid_height();valid&=spouse.has_valid_gender();return valid;};GroupCIProduct.prototype.requires_gender=function(){return true;};GroupCIProduct.prototype.requires_height=function(){return true;};GroupCIProduct.prototype.requires_weight=function(){return true;};GroupCIProduct.prototype.requires_is_smoker=function(){return true;};GroupCIProduct.prototype.has_critical_illness_coverages=function(){return true;};GroupCIProduct.prototype.should_show_step_5=function(){return false;};GroupCIProduct.prototype.should_use_date_of_hire_for_identity=function(){return false;};GroupCIProduct.prototype.is_fpp_product=function(){return false;};function FPPGovProduct(product_data){Product.call(this);this.product_type="FPP-Gov";this.product_data=product_data;}
...
$('#bank-draft-form').validate({errorElement:'div',errorClass:'help-block',focusInvalid:false,highlight:wizard_validate_highlight,success:wizard_validate_success,errorPlacement:bank_draft_error_placement,rules:{'bank-account-holder-name':{required:{depends:ui.requires_bank_info}},'bank-account-type':{required:{depends:ui.requires_bank_info}},'bank-account-number':{required:{depends:ui.requires_bank_info},digits:{depends:ui.requires_bank_info}},'bank-routing-number':{required:{depends:ui.requires_bank_info},digits:{depends:ui.requires_bank_info}},'bank-name':{required:{depends:ui.requires_bank_info}},'bank-street-one':{required:{depends:ui.requires_bank_info}},'bank-city':{required:{depends:ui.requires_bank_info}},'bank-state':{required:{depends:ui.requires_bank_info}},'bank-zip':{required:{depends:ui.requires_bank_info}}}});}
