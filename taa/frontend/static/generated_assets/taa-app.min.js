...
ko.components.register('limited-state-select',{viewModel:function(params){this.limiter=params.limiter;},template:'<select name="enrollmentState" id="enrollmentState" data-bind="\
            ...
            "></select>'});ko.components.register('limited-product-select',{viewModel:function(params){this.limiter=params.limiter;},template:'<select name="productID" id="productID" data-bind="\
...
ko.applyBindingsToNode(option,{disable:ko.computed(function(){return self.is_state_disabled(state);})},state);};self.selected_state.subscribe(function(state){var valid_products=_.without(self.selected_products(),function(product){return!self.is_valid_product_for_state(product,state);});self.selected_products(valid_products);});self.enabled_states=ko.computed(function(){return _.filter(self.available_states,function(state){return _.all(self.selected_products(),function(product){return self.is_valid_product_for_state(product,state);});});});self.enabled_products=ko.computed(function(){if(self.selected_state()!==null){return _.filter(self.available_products(),function(product){return self.is_valid_product_for_state(product,self.selected_state());});}
