...
if(window.vm.requires_actively_at_work()){if(window.vm.is_employee_actively_at_work()===null){return false;}}
...
var health_questions=(function(){'use strict';function ProductHealthQuestions(product_coverage,spouse_questions,all_health_questions,employee_questions,applicant_list,omit_actively_at_work){var self=this;self.product_coverage=product_coverage;self.applicant_list=applicant_list;self.employee=_.find(applicant_list.applicants(),function(applicant){return applicant.type===wizard_applicant.Applicant.EmployeeType;});self.spouse=_.find(applicant_list.applicants(),function(applicant){return applicant.type===wizard_applicant.Applicant.SpouseType;});self.health_questions=ko.pureComputed(function(){var questions=[];var emp_questions=health_questions.process_employee_question_data(employee_questions,self.product_coverage,self.employee,omit_actively_at_work);var sp_questions=health_questions.process_spouse_question_data(spouse_questions,self.product_coverage,self.spouse,omit_actively_at_work);var soh_questions=health_questions.process_health_question_data(all_health_questions,self.product_coverage);$.merge(questions,emp_questions);$.merge(questions,sp_questions);$.merge(questions,soh_questions);return questions;});self.show_yes_dialogue=function(applicant,message){var button_options={remove:{label:"Remove this applicant",className:'btn-danger',callback:function(){var applicant_coverage_options=self.product_coverage.get_coverage_options_for_applicant(applicant);if(applicant.type===wizard_applicant.Applicant.ChildType){self.product_coverage.applicant_list.remove_applicant(applicant);}else{var null_option=_.find(applicant_coverage_options,function(o){return o.face_value===0;});self.product_coverage.__get_coverage_for_applicant(applicant).customized_coverage_option(null_option);}
...
function process_applicant_question_data(question_data_by_product,product_coverage,applicant,omit_actively_at_work){var questions=[];var product_data=product_coverage.product.product_data;var omit_actively_at_work_question=typeof omit_actively_at_work!=='undefined'?!!omit_actively_at_work:false;_.each(question_data_by_product,function(product_questions,product_id){if(product_id==product_data.id){var question_factory;if(product_coverage.product.product_data.is_guaranteed_issue){question_factory=function(question_data){if(question_data.label==='Employee Actively at Work'){return new ActivelyAtWorkGiHealthQuestion(product_coverage.product,question_data,product_coverage,product_data.gi_criteria,product_data.statement_of_health_bypass_type,product_data.bypassed_soh_questions,applicant);}
return new GIHealthQuestion(product_coverage.product,question_data,product_coverage,product_data.gi_criteria,product_data.statement_of_health_bypass_type,product_data.bypassed_soh_questions);};}else{question_factory=function(question_data){return new StandardHealthQuestion(question_data,product_coverage);}}
questions=_.chain(product_questions).filter(function(question){if(question.label==='Employee Actively at Work'){if(product_data.is_guaranteed_issue){return!omit_actively_at_work_question;}
return false;}
