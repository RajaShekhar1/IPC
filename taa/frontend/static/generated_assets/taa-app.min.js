...
Applicant.EmployeeType="employee";Applicant.SpouseType="spouse";Applicant.ChildType="children";var ApplicantGroup=function(options,applicants){var self=this;self.type=options.type;self.applicants=applicants;self._id=_applicant_count++;self.is_valid=ko.computed(function(){return _.all(self.applicants(),function(applicant){return applicant.is_valid();});});self.name=ko.pureComputed(function(){var non_empty_name_applicants=_.filter(self.applicants(),function(applicant){return $.trim(applicant.name())!=="";});return _.invoke(non_empty_name_applicants,"name").join(", ");});self.is_age_in_band=function(min_age,max_age){return self.all_ages_in_band(min_age,max_age);};self.all_ages_in_band=function(min_age,max_age){var is_in_band=true;if(min_age!==null){is_in_band=is_in_band&&_.all(self.applicants(),function(applicant){return applicant.get_age()>=min_age;});}
...
if(is_valid){$(document.body).scrollTop(0);}}
