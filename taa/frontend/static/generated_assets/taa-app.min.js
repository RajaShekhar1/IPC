...
function handle_error_and_retry(req,wizard_results){handle_remote_error_with_retry(req,function retry_callback(success){if(success){_send_wizard_results(wizard_results);}});}
var SUBMISSION_RETRIES_ALLOWED=3;var submission_retry_count=0;function handle_remote_error_with_retry(response,retry_callback){if(response.status===401){}else if(response.status===503){if(submission_retry_count<SUBMISSION_RETRIES_ALLOWED){submission_retry_count+=1;setTimeout(function(){retry_callback(true);},8000);}else{alert("Sorry, an error occurred communicating with the server. Please check your Agent Inbox to sign this application.");window.location=urls.get_manage_case_url(window.vm.options.case_data.id);}}else if(response.status===500){alert("Sorry, the server encountered an error processing this request.");}else{alert("Sorry, an error occurred communicating with the server. Please check your connection to the internet and try again.");}}
