...
return self.recommended_coverage.premium;};self.format_premium_option=function(){return self.recommended_coverage.format_premium_option()};self.format_coverage=function(){return self.recommended_coverage.format_face_value();};}
...
$.ajax(url,options);}
return{WizardVM:WizardVM}})();var agent_inbox=(function(){var AgentInboxViewModel=function(){var self=this;self.is_loading=ko.observable(true);self.envelopes=ko.observableArray([]);self.sign_envelope=function(){var envelope_id=$(this).attr("data-id");$.post("/envelopes/"+envelope_id+"/sign").success(self.handle_signing_redirect).error(self.handle_signing_failure);bootbox.alert("Redirecting to signing page, please wait...");};self.handle_signing_redirect=function(data){window.location.href=data.data.url;};self.handle_signing_failure=function(data){bootbox.hideAll();bootbox.alert("There was a problem redirecting to the signing page.");};self.table_options={order:[[2,"desc"]],columns:[{sortable:false,data:function(row){return'<button class="btn btn-primary btn-xs sign-button" data-id="'+row.id+'"><span class="ace-icon glyphicon glyphicon-pencil"></span> Sign</button>';}},{data:"timestamp",visible:false},{data:"formatted_timestamp",orderData:[1]},{data:"group"},{data:"employee_first"},{data:"employee_last"},{data:"products"},{data:"coverage"}]};fetch_envelopes(self.is_loading,self.envelopes);$("body").on("click","button.sign-button",self.sign_envelope);};var EnvelopeViewModel=function(envelope){var self=this;self.id=envelope.id;self.timestamp=envelope.timestamp;self.formatted_timestamp=moment(envelope.timestamp).format("MM/DD/YYYY h:mma");self.group=envelope.group;self.employee_first=envelope.employee_first;self.employee_last=envelope.employee_last;self.products=envelope.products;self.coverage=envelope.coverage;};function fetch_envelopes(loading_observable,envelopes_observable){loading_observable(true);$.ajax("/envelopes").success(function(data){loading_observable(false);var envelopes=_.map(data.data,function(envelope_data){return new EnvelopeViewModel(envelope_data);});envelopes_observable(envelopes);})}
return{init_viewmodel:function(params){return new AgentInboxViewModel(params);}}})();var case_management=(function(){var loading_html="<span class='icon-spinner icon-spin grey bigger-200'></span> <span class='bigger-175'> Loading data...</span>";function refresh_census_table(case_id,url,table_selector,loading_selector,init_callback,no_data_cb,success_callback){var loading=$(loading_selector);var table=$(table_selector);var table_settings={"responsive":{breakpoints:get_responsive_datatables_breakpoints()},"autoWidth":false,"aoColumnDefs":[{"aTargets":[0],"bSortable":false,"mData":function(source){if(source.enrollment_status!==null){return'<button class="btn btn-primary btn-xs enroll-employee" data-id="'+source.id+'"><span class="ace-icon glyphicon glyphicon-plus"></span> Add Coverage</button>';}
return'<button class="btn btn-primary btn-sm enroll-employee" data-id="'+source.id+'">Enroll</button>';}},{"aTargets":[1],"mData":function(source){return format_enrollment_status_html(source.enrollment_status);}},{"aTargets":[2],"mData":function(source){return"<a href='/enrollment-case/"+source.case_id+"/census/"+source.id+"'>"+source.employee_first+"</a>";}},{"aTargets":[3],"mData":function(source){return"<a href='/enrollment-case/"+source.case_id+"/census/"+source.id+"'>"+source.employee_last+"</a>";}},{"aTargets":[4],"mData":function(source){return normalize_date(source.employee_birthdate);}},{"aTargets":[5],"mData":function(source){return source.email;}}],"aaSorting":[[3,"asc"]]};loading.html(loading_html);clear_table(table);$.get(url,{},function(resp){loading.html("");if($.fn.DataTable.fnIsDataTable(table[0])){table.dataTable().fnAddData(resp.data);if(success_callback!==undefined){success_callback(table,resp.data);}}else if(resp.data.length>0){table.show();$(".no-census-header").hide();table_settings.aaData=resp.data;table.wrap("<div class='dataTables_borderWrap' />").dataTable(table_settings);if(init_callback!==undefined){init_callback(table,resp.data);}
