...
var case_management=(function(){var loading_html="<span class='icon-spinner icon-spin grey bigger-200'></span> <span class='bigger-175'> Loading data...</span>";function refresh_census_table(case_id,url,table_selector,loading_selector,table_options,init_callback,no_data_cb){var loading=$(loading_selector);var table=$(table_selector);var table_defaults={"responsive":{breakpoints:get_responsive_datatables_breakpoints()},"autoWidth":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0],"mData":function(source){return"<a href='/manage-case/"+case_id+"/census/"+source.id+"'><span class='glyphicon glyphicon-edit'></span></a>";}},{"aTargets":[1],"mData":"employee_first"},{"aTargets":[2],"mData":"employee_last"},{"aTargets":[3],"mData":function(source){return format_date(parse_date(source.employee_birthdate));},className:"min-breakIV"},{"aTargets":[4],"mData":"employee_email",className:"min-breakII"},{"aTargets":[5],"mData":function(source){return format_enrollment_status_html(source.enrollment_status);},className:"min-breakV"}],"aaSorting":[[2,"asc"]],"iDisplayLength":25};var table_settings=$.extend({},table_defaults,table_options||{});loading.html(loading_html);clear_table(table);$.get(url,{},function(resp){loading.html("");if($.fn.DataTable.fnIsDataTable(table.get(0))){table.dataTable().fnAddData(resp.data);}else if(resp.data.length>0){table.show();$(".no-census-header").hide();table_settings.aaData=resp.data;table.wrap("<div class='dataTables_borderWrap' />").dataTable(table_settings);if(init_callback!==undefined){init_callback(table);}}else if(no_data_cb!==undefined){no_data_cb();}});}
...
return{refresh_census_table:refresh_census_table,refresh_enrollments_table:refresh_enrollments_table};})();if(typeof Object.create!='function'){(function(){var F=function(){};Object.create=function(o){if(arguments.length>1){throw Error('Second argument not supported');}
...
function WizardUI(defaults){var self=this;self.defaults=defaults;self.insurance_product=build_product(self,defaults.products);self.disclaimer_notice_confirmed=ko.observable(false);self.did_decline=ko.observable(false);self.is_in_person_application=ko.observable('is_in_person'in defaults&&defaults.is_in_person);self.identityToken=ko.observable("");self.identityType=ko.observable("");self.enrollCity=ko.observable(defaults.enroll_city||"");self.enrollState=defaults.state;self.was_state_provided=("state"in defaults&&defaults.state!==null&&defaults.state!="XX");self.company_name=ko.observable(defaults.company_name||"(Unknown Company)");self.existing_insurance="no";self.replacing_insurance="no";self.policy_owner=ko.observable("self");self.other_owner_name=ko.observable("");self.other_owner_ssn=ko.observable("");self.spouse_policy_owner=ko.observable("employee");self.spouse_other_owner_name=ko.observable("");self.spouse_other_owner_ssn=ko.observable("");self.employee_beneficiary=ko.observable("spouse");self.spouse_beneficiary=ko.observable("employee");self.employee_beneficiary_name=ko.observable("");self.employee_beneficiary_relationship=ko.observable("");self.employee_beneficiary_ssn=ko.observable("");self.employee_beneficiary_dob=ko.observable("");self.spouse_beneficiary_name=ko.observable("");self.spouse_beneficiary_relationship=ko.observable("");self.spouse_beneficiary_ssn=ko.observable("");self.spouse_beneficiary_dob=ko.observable("");self.selected_plan=ko.observable(new NullBenefitsPackage(self));var question_data=process_health_question_data(self,defaults.health_questions,self.insurance_product.product_data);self.health_questions=ko.observableArray(question_data);self.selected_recommendation=ko.observable(null);self.employee=ko.observable(new InsuredApplicant(InsuredApplicant.EmployeeType,self.defaults.employee_data||{},self.selected_plan,self.health_questions));self.should_show_gender=function(){return self.insurance_product.requires_gender();};self.should_show_height=function(){return self.insurance_product.requires_height();};self.should_show_weight=function(){return self.insurance_product.requires_weight();};self.should_show_smoker=function(){return self.insurance_product.requires_is_smoker();};self.should_show_extended_questions=function(){return(self.should_show_gender()||self.should_show_height()||self.should_show_weight()||self.should_show_smoker());};var spouse_data=self.defaults.spouse_data||{};if(!spouse_data.last){spouse_data.last=self.employee().last();}
...
return names;}else{return"";}});self.add_child=function(){var child_insured_applicant=new InsuredApplicant(InsuredApplicant.ChildType,{last:self.employee().last()||""},self.selected_plan,self.health_questions);self.children.push(child_insured_applicant);if(self.children().length>3){$("#addChildBtn").hide();}
...
return self.insurance_product.get_coverage_options_for_applicant('children')();});self.is_show_rates_clicked=ko.observable(false);self.is_rate_table_loading=ko.observable(false);self.is_employee_info_valid=ko.computed(function(){return self.insurance_product.is_valid_employee(self.employee());});self.can_display_rates_table=ko.computed(function(){var valid=self.employee().is_valid();valid&=self.is_employee_info_valid();if(self.is_show_rates_clicked()){self.validator.form();}
...
var valid_options=[new NullBenefitOption()];var rate_choices=[];$.each(rates.weekly_byface,function(){var rate=this;if(rate.coverage%5000==0){rate_choices.push(rate);}});$.merge(valid_options,$.map(rate_choices,convert_rate_to_benefit_option));self.all_coverage_options[applicant_type](valid_options);}
if(applicant_type=="spouse"&&rates.weekly_byface!==undefined){var demographic_spouse_rates=[];var all_spouse_rates=[];$.each(rates.weekly_byface,function(){var rate=this;if(rate.coverage%5000==0&&rate.coverage<=25000){demographic_spouse_rates.push(rate);}
...
BenefitOption.display_benefit_option=function(item){return item.format_for_dropdown();};function CIBenefitOption(wrapped_option){var self=this;self.is_by_face=wrapped_option.is_by_face;self.weekly_premium=wrapped_option.weekly_premium;self.face_value=wrapped_option.face_value;self.format_weekly_premium=wrapped_option.format_weekly_premium;self.format_weekly_premium_option=wrapped_option.format_weekly_premium_option;self.format_face_value=function(){var face_value_formatted=format_face_value(self.face_value);var ci_value=Math.round(self.face_value*.3);var ci_value_formatted=format_face_value(ci_value);return face_value_formatted+"<br><small>("+ci_value_formatted+" CI)</small>";};self.format_for_dropdown=wrapped_option.format_for_dropdown;self.is_valid=wrapped_option.is_valid;self.serialize_data=wrapped_option.serialize_data;}
function NullBenefitOption(){var self=this;self.is_by_face=true;self.weekly_premium=0;self.face_value=0;self.is_valid=function(){return false;};self.format_weekly_premium_option=function(){return"";};self.format_weekly_premium=function(){};self.format_face_value=function(){return"- no benefit -";};self.format_for_dropdown=function(){return"- no benefit -";};self.serialize_data=function(){return{}}}
...
function are_health_questions_valid(){var el;if(ui.should_show_other_insurance_questions()&&ui.is_in_person_application()&&general_questions_by_id['existing_insurance'].get_val()===null){return false;}
if(ui.should_show_other_insurance_questions()&&!ui.is_in_person_application()&&general_questions_by_id['existing_insurance_remote'].get_val()!="No"){return false;}
if(ui.should_show_other_insurance_questions()&&general_questions_by_id['replace_insurance'].get_val()!="No"){return false;}
...
wizard_results['product_data']=ui.insurance_product.product_data;ajax_post("/submit-wizard-data",{"wizard_results":wizard_results},function(resp){if(resp.error){bootbox.dialog({message:"There was a problem generating the application form ("+resp.error+").  Please contact the enrollment system administrator.",buttons:{"success":{"label":"OK","className":"btn-sm btn-primary"}}});}else{location=resp.redirect}},handle_remote_error,true);bootbox.dialog({message:"Generating application form for signature, please wait...",buttons:{"success":{"label":"Close","className":"btn-sm btn-primary"}}});}
...
function send_form_data(method,url,data,on_success,on_error){return submit_data(method,url,data,true,on_success,on_error);}
...
function normalize_date(date_str){return format_date(parse_date(date_str));}
...
$(".submit-message").addClass("error").html("Please fix the indicated problems and resubmit.").show();$.each(all_errors,function(field_name,errors){show_errors(field_name,errors);return false;});focus_first_error(all_errors);}
...
ko.components.register('limited-state-select',{viewModel:function(params){self.limiter=params.limiter;},template:'<select name="enrollmentState" id="enrollmentState" data-bind="\
            ...
            "></select>'});ko.components.register('limited-product-select',{viewModel:function(params){self.limiter=params.limiter;},template:'<select name="productID" id="productID" data-bind="\
          ...
          </select>'});var ProductStatesLimiterViewModel=function(product_state_mapping,selected_state,available_states,selected_products,available_products){var self=this;var states_for_products={};_.each(product_state_mapping,function(statecodes,product_id){states_for_products[product_id]=[];_.each(statecodes,function(statecode){var matched_state=_.find(available_states,function(s){return s.statecode==statecode;});if(matched_state){states_for_products[product_id].push(matched_state);}});});self.product_state_mapping=states_for_products;self.selected_state=selected_state;self.available_states=available_states;self.selected_products=selected_products;self.available_products=available_products;self.selected_product=ko.computed({read:function(){if(self.selected_products().length>0){return self.selected_products()[0];}else{return null;}},write:function(val){if(val){self.selected_products([val]);}else{self.selected_products([]);}},owner:self});self.is_valid_product_for_state=function(product,state){return _.contains(self.product_state_mapping[product.id],state);};self.is_state_disabled=function(state){return!_.contains(self.enabled_states(),state);};self.is_product_disabled=function(product){return!_.contains(self.enabled_products(),product);};self.disable_product_option_if_invalid=function(option,product){if(product===undefined){return;}
...
ko.applyBindingsToNode(option,{disable:ko.computed(function(){return self.is_state_disabled(state);})},state);};self.selected_state.subscribe(function(state){var valid_products=_.without(self.selected_products(),function(product){return self.is_valid_product_for_state(product,state);});self.selected_products(valid_products);});self.enabled_states=ko.computed(function(){return _.filter(self.available_states,function(state){return _.all(self.selected_products(),function(product){return self.is_valid_product_for_state(product,state);});});});self.enabled_products=ko.computed(function(){if(self.selected_state()==null){return self.available_products();}else{return _.filter(self.available_products(),function(product){return self.is_valid_product_for_state(product,self.selected_state());});}});};