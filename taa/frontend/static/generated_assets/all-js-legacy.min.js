...
coverage_option.format_premium()+' for '+coverage_option.format_face_value()+' coverage. If you need to see alternative amounts please return to step 1.');}});});};self.on_smoker_status_changed_dialog=function(){$('#smoking-status-dialog').modal('hide');};function subscribe_to_boolean_changes(observable,callback){if(!observable){return;}
...
function update_rates_if_height_weight_are_valid(){var employee_height=self.employee().height();var employee_weight=self.employee().weight();if(employee_weight&&typeof employee_weight==='string'){employee_weight=parseInt(employee_weight);}
var is_employee_height_and_weight_valid=_.isNumber(employee_height)&&_.isNumber(employee_weight);var is_spouse_height_and_weight_valid=true;if(self.spouse()){var spouse_height=self.spouse().height();var spouse_weight=self.spouse().weight();if(spouse_weight&&typeof spouse_weight==='string'){spouse_weight=parseInt(spouse_weight);}
