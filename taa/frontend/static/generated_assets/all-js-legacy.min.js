...
function observe_enrollment_upload_form_submit(){console.log("here");$("#enrollment-csv-form").on("submit",function(){var form=this;var file_select=$(this).find("input[type=file]").get(0);var files=file_select.files;var form_data=new FormData();if(files.length!==1){alert("Please select a file");return false;}
...
self.toggle_enrollment_buttons();});self.toggle_enrollment_buttons=function(){if(self.is_active()){$("button.enroll-employee").prop('disabled',false);}else{$("button.enroll-employee").prop('disabled',true)}};self.toggle_enrollment_buttons();self.switch_label=ko.computed(function(){if(self.is_active()){return"Case is Active";}else{return"Case is Not Active";}});self.get_form_error=ko.computed(function(){if(self.company_name.is_unique!==undefined&&!self.company_name.is_unique()){return"The name '"+self.company_name()+"' is already used."}
return"";});self.check_unique_name=function(current_value,callback){$.get(urls.get_cases_api_url(case_data.id),{by_name:current_value},function(result){var is_unique=(result.data.length==0||current_value==case_data.company_name);callback(is_unique);},"json");};self.validate=function(){hide_all_errors();_.invoke(self.annual_enrollment_periods(),"error","");var unique_name_error=self.get_form_error();if(unique_name_error!==""){add_case_error(errors,"company_name",unique_name_error);}
var errors={};if($.trim(self.company_name())==""){add_case_error(errors,"company_name","Company name is required.");}
...
var case_request=send_json_data("PUT",urls.get_case_api_url(case_data.id),self.serialize_case());var periods_request=send_json_data("PUT",urls.get_case_api_enrollment_periods_url(case_data.id),self.serialize_enrollment_periods());if(self.is_self_enrollment()){var self_enroll_request=send_json_data("PUT",urls.get_case_api_self_enrollment_url(case_data.id),self.serialize_self_enroll());}
$('#save-success').hide();$('#save-fail').hide();var on_success=function(case_xhr,periods_xhr,self_enroll_xhr){self.is_data_dirty(false);self.flash_messages.flash_success("Data Saved");$("#save-success").text("Data Saved");self.loading_modal(null);};var on_failure=function(failed_xhr){var errors=failed_xhr.responseJSON.errors||[];show_all_errors(errors);self.loading_modal(null);self.flash_messages.flash_error("Save failed. Please correct any errors below.");};if(!self.is_self_enrollment()){var done=$.when(case_request,periods_request);}else{var done=$.when(case_request,periods_request,self_enroll_request);}
...
var valid=true;$.each(window.ui.selected_plan().get_all_covered_people(),function(){var covered_person=this;$.each(covered_person.health_questions(),function(){if(this.button_group()&&this.button_group().is_required()&&((this.question.is_ignored&&this.button_group().get_val()===null)||(!this.question.is_ignored&&this.button_group().get_val()!=="No"))){valid=false;return false;}});if(!valid){return false;}});return valid;}
...
function process_spouse_question_data(root,question_data_by_product,product_data){var questions=[];_.each(question_data_by_product,function(product_questions,product_id){if(product_id==product_data.id){var question_factory;if(product_data.is_guaranteed_issue){question_factory=function(question_data){return new GIHealthQuestion(root.insurance_product,question_data,root.selected_plan,product_data.gi_criteria,product_data.statement_of_health_bypass_type,product_data.bypassed_soh_questions);};}else{question_factory=function(question_data){return new StandardHealthQuestion(question_data,root.selected_plan);}}
...
var enrollment_wizard=(function(){return{init:function(data){console.log("init with args: ",arguments);var ui=new wizard_viewmodel.WizardUI(data);window.ui=ui;ko.applyBindings(ui);init_validation();}};})();var payment_mode=(function(){function PaymentMode(options){this.frequency=options.frequency;this.label=options.label;}
PaymentMode.prototype={serialize:function(){return this.frequency;}};return{create_payment_mode:function(options){return new PaymentMode(options);},select_initial_payment_mode:function(case_data,payment_modes){var initial_payment_mode=null;if(case_data&&case_data.payment_mode>=0){initial_payment_mode=_.find(payment_modes,function(pm){return pm.frequency===case_data.payment_mode;});if(initial_payment_mode===undefined){initial_payment_mode=null;}}
return initial_payment_mode;},can_change_payment_mode:function(case_data){return(case_data.payment_mode===-1||!case_data.payment_mode);}}})();var product_rates_service=(function(){function get_product_rates(products,payment_mode,applicant_list,callback,error_callback){var requests=_.map(products,function(product){return $.ajax({url:"/products/"+product.product_data.id+"/rates",dataType:"json",method:"POST",contentType:"application/json; charset=utf-8",processData:false,data:JSON.stringify(build_rate_parameters(payment_mode,applicant_list))});});$.when.apply($,requests).done(callback).fail(error_callback);}
function build_rate_parameters(payment_mode,applicant_list){var params={payment_mode:payment_mode.frequency};return $.extend({},params,build_applicant_parameters(applicant_list));}
function build_applicant_parameters(applicant_list){return{employee:applicant_list.get_employee().serialize_data(),spouse:(applicant_list.has_valid_spouse())?applicant_list.get_spouse().serialize_data():null,num_children:applicant_list.get_valid_children().length};}
return{get_product_rates:get_product_rates};})();var wizard_products=(function(){function build_products(root,products){if(products.length==0){alert("Error: No products to enroll.");return null;}
return _.map(products,function(product){return build_product(root,product);})}
...
return true;}
_.each(plan.get_covered_applicants_with_type(),function(val){var applicant_type={"Employee":"employee","Spouse":"spouse","Child":"children"}[val.type];var can_apply=validate_coverage_amount(val.applicant,applicant_type,val.coverage);if(!can_apply){is_valid=false;}});if(!is_valid){e.preventDefault();}}
...
if(window.ui.has_contingent_beneficiary_error()){e.preventDefault();return;}}
if(info.step==6&&info.direction=='next'){if(!$('#step6-form').valid()){e.preventDefault();return;}}}).on('finished.fu.wizard',function(e){if(!$('#step6-form').valid())return false;if(window.ui.insurance_product.should_confirm_disclosure_notice()&&!window.ui.disclaimer_notice_confirmed()){bootbox.dialog({message:"Please confirm that you have received the disclosure notice.",buttons:{"danger":{"label":"OK","className":"btn-warning"}}});return false;}
...
var wizard_viewmodel=(function(){function CoverageVM(available_products,applicant_list){this.available_products=available_products;this.applicants=applicant_list;this.product_coverage_viewmodels=ko.observableArray(_.map(available_products,function(p){return new ProductCoverageViewModel(p,applicant_list);}));this.current_product=ko.observable(this.product_coverage_viewmodels()[0]);this.has_multiple_products=ko.pureComputed(function(){return this.product_coverage_viewmodels().length>1;},this);this.previous_product=ko.pureComputed(function(){var index=this.product_coverage_viewmodels().indexOf(this.current_product());return val_or_null(this.product_coverage_viewmodels(),index-1);},this);this.next_product=ko.pureComputed(function(){var index=this.product_coverage_viewmodels().indexOf(this.current_product());return val_or_null(this.product_coverage_viewmodels(),index+1);},this);function val_or_null(array,index){if(index>=0&&index<index.length){return array[index];}
return null;}}
function ProductCoverageViewModel(product,applicant_list){var self=this;self.product=product;self.applicant_list=applicant_list;self.available_recommendations=ko.observableArray(null);self.selected_recommendation=ko.observable(new NullRecommendation({}));self._applicant_coverage_viewmodels={};self.applicant_coverage_selections=ko.pureComputed(this._get_applicant_coverage_selections,this);}
ProductCoverageViewModel.prototype={get_coverage_selection_template:function(){return'recommendation_coverage_selection';},select_recommended_coverage:function(recommendation){this.selected_recommendation(recommendation);_.each(this.applicant_coverage_selections(),function(applicant_coverage){applicant_coverage.select_recommended_coverage(recommendation);},this);},_get_applicant_coverage_selections:function(){var selections=[];if(this.applicant_list.has_valid_employee()){selections.push(this.get_coverage_for_applicant(this.applicant_list.get_employee()));}
if(this.applicant_list.has_valid_spouse()){selections.push(this.get_coverage_for_applicant(this.applicant_list.get_spouse()));}
if(this.applicant_list.has_valid_children()){selections.push(this.get_coverage_for_applicant(this.applicant_list.get_children_group()));}
return selections;},get_coverage_for_applicant:function(applicant){if(applicant.type==wizard_applicant.Applicant.ChildType){applicant=this.applicant_list.get_children_group();}
if(applicant._id in this._applicant_coverage_viewmodels){return this._applicant_coverage_viewmodels[applicant._id];}else{var new_selection_instance=new ApplicantCoverageSelectionVM(applicant,this.product);this._applicant_coverage_viewmodels[applicant._id]=new_selection_instance;return new_selection_instance;}},get_applicant_recommendations:function(recommendation){return _.map(this.applicant_coverage_selections(),function(applicant_coverage){return new ApplicantRecommendationVM(applicant_coverage.applicant,recommendation);},this);}};function ApplicantRecommendationVM(applicant,recommendation){this.applicant=applicant;this.recommendation=recommendation;this.get_coverage_recommendation=function(){if(this.recommendation===null){return new NullBenefitOption();}else{return this.recommendation[applicant];}};this.is_valid=function(){return this.get_coverage_recommendation().is_valid();};}
function ApplicantCoverageSelectionVM(applicant,product){this.applicant=applicant;this.product=product;this.coverage_option=ko.observable(new NullBenefitOption());this.riders=ko.observableArray();this.beneficiaries=ko.observableArray([]);}
ApplicantCoverageSelectionVM.prototype={select_recommended_coverage:function(recommendation){var coverage_option=recommendation[this.applicant.type];if(coverage_option){this.coverage_option(coverage_option);}},select_custom_coverage:function(option){this.coverage_option(option);},has_selected_coverage:function(){return this.coverage_option().is_valid();},get_selected_coverage:function(){return this.coverage_option();},format_name:function(){return this.applicant.name();}};function WizardVM(options){var self=this;self.enrollment_case=options.case_data;self.products=wizard_products.build_products(self,options.products);self.is_in_person_application=(options.enrollment_type==="inperson");self.is_rate_table_loading=ko.observable(false);self.is_show_rates_clicked=ko.observable(false);init_applicants();init_jquery_validator();self.coverage_vm=new CoverageVM(self.products,self.applicant_list);self.product_coverage_viewmodels=self.coverage_vm.product_coverage_viewmodels;var should_show_spouse_initially=self.spouse().any_valid_field();self.should_show_spouse=ko.observable(should_show_spouse_initially);var should_show_children_initially=false;self.should_include_children=ko.observable(should_show_children_initially);self.add_child=function(){var child=wizard_applicant.create_applicant({last:self.employee().last(),type:wizard_applicant.Applicant.ChildType});self.applicant_list.add_applicant(child);};self.remove_child=function(child){self.applicant_list.remove_applicant(child);if(self.applicant_list.get_valid_children().length===0){self.should_include_children(false);}};self.rendered_child=function(element){$(element).hide().slideDown(400);};self.removing_child=function(element){$(element).slideUp(function(){$(element).remove();});};self.is_recommended_table_visible=ko.computed(function(){return(self.is_show_rates_clicked()&&self.can_display_rates_table());});self.update_rate_table=function(){if(!self.is_rate_table_loading()){self.is_rate_table_loading(true);}
self.is_show_rates_clicked(true);if(!self.is_recommended_table_visible()){self.is_rate_table_loading(false);return;}
self.refresh_rate_table();};self.refresh_rate_table=function(){self.is_rate_table_loading(true);product_rates_service.get_product_rates(self.products,self.payment_mode(),self.applicant_list,self.show_updated_rates,self.handle_update_rates_error);};self.show_updated_rates=function(){for(var i=0;i<arguments.length;i++){var product_rate_response=arguments[i][0];var product_rates=product_rate_response.data;}
self.is_rate_table_loading(false);};self.handle_update_rates_error=function(resp){if(resp.status===400&&resp.responseJSON&&resp.responseJSON.errors){$.each(resp.responseJSON.errors,function(){var field_name=this.field;var message=this.error;limit_error_lookup[field_name](true);self.validator.form();});}else{handle_remote_error();}};self.should_include_spouse_in_table=ko.computed(function(){return self.should_show_spouse()&&self.spouse().is_valid();});self.show_spouse_name=ko.computed(function(){return(self.should_include_spouse_in_table())?self.spouse().name():"";});self.should_include_children_in_table=ko.computed(function(){return(self.should_include_children()&&self.applicant_list.has_valid_children()&&self.are_children_ages_valid());});self.payment_modes=_.map(options.payment_modes||[],function(pm){return payment_mode.create_payment_mode(pm);});self.payment_mode=ko.observable(payment_mode.select_initial_payment_mode(options.case_data,self.payment_modes));self.can_change_payment_mode=function(){return payment_mode.can_change_payment_mode(options.case_data);};self.is_payment_mode_valid=ko.pureComputed(function(){return((!self.can_change_payment_mode())||(self.can_change_payment_mode()&&self.payment_mode()!==null));});self.has_show_rates_been_clicked=ko.observable(false);self.can_display_rates_table=ko.computed(function(){return(self.applicant_list.has_valid_employee()&&self.is_payment_mode_valid());});self.show_coverage_selection_table=function(){if(!self.can_display_rates_table()){return;}
var valid_form=true;if(self.validator){valid_form=self.validator.form();}
if(valid_form){self.has_show_rates_been_clicked(true);}
self.update_rate_table();};self.is_coverage_selection_visible=ko.pureComputed(function(){return(self.has_show_rates_been_clicked()&&self.can_display_rates_table());});self.should_display_show_rates_button=ko.pureComputed(function(){return!self.is_coverage_selection_visible();});self.should_show_critical_illness_styling=function(){return false;};self.should_show_extended_questions=function(){return(any_product('requires_gender')||any_product('requires_height')||any_product('requires_weight')||any_product('requires_is_smoker'));};var any_product=function(method_name){return _.any(_.invoke(self.products,'requires_gender'));};function min_emp_age(){return _.min(_.invoke(self.products,'min_emp_age'));}
...
self.can_decline=ko.computed(function(){return true;});self.did_decline=ko.observable(false);self.is_selection_error_visible=ko.observable(false);self.exit_application=function(){};self.warning_modal_title=ko.observable("");self.warning_modal_body=ko.observable("");self.show_warning_modal=function(title,body){self.warning_modal_title(title);self.warning_modal_body(body);$("#warning_modal").modal('show');};function init_applicants(){var initial_applicant_list=_.map(options.applicants||[],function(applicant_data){return wizard_applicant.create_applicant(applicant_data);});self.applicant_list=new wizard_applicant.ApplicantList(initial_applicant_list);self.employee=function(){return self.applicant_list.get_employee();};self.spouse=function(){return self.applicant_list.get_spouse();};self.children=ko.computed(function(){return self.applicant_list.get_children();});var num_initial_children=self.children().length;var last_name=self.employee().last()||"";if(num_initial_children<2){var num_blank_children_forms=(2-num_initial_children);for(var i=0;i<num_blank_children_forms;i+=1){var ch=wizard_applicant.create_applicant({last:last_name,type:wizard_applicant.Applicant.ChildType});self.applicant_list.add_applicant(ch);}}}
...
function ApplicantProductSelection(root,product){var self=this;self.root=root;self.insurance_product=product;self.did_decline=ko.observable(false);self.selected_plan=ko.observable(new NullBenefitsPackage(self));var questions=process_spouse_question_data(self,defaults.spouse_questions,self.insurance_product.product_data);var soh_questions=process_health_question_data(self,defaults.health_questions,self.insurance_product.product_data);$.merge(questions,soh_questions);self.health_questions=ko.observableArray(questions);self.selected_recommendation=ko.observable(null);self.get_replacement_paragraphs=function(){var paragraph_map=self.insurance_product.get_replacement_paragraphs();var paragraphs=paragraph_map[self.root.enrollState];if(!paragraphs){return[];}
return paragraphs;};}
