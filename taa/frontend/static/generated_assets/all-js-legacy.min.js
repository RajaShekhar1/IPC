...
var start=self.get_open_enrollment_period().start_date();var end=self.get_open_enrollment_period().end_date();return today_between(start,end);});self.annual_enrollment_periods=ko.computed(function(){return _.filter(self.enrollment_periods(),function(p){return p.period_type==="annual_period";});});self.annual_today_between=ko.computed(function(){if(self.annual_enrollment_periods().length>=4&&self.annual_enrollment_periods().first().start_date()!==""){return self.annual_enrollment_periods().reduce(function(a,period){var start_date=moment(period.start_date(),"MM/DD");var end_date=moment(period.end_date(),"MM/DD");return today_between(start_date,end_date)||a;},false);}
...
self.owner_agent=ko.computed(function(){return settings.active_agents.find(function(elem){return elem.id===parseInt(self.owner_agent_id(),10);});});self.partner_agent_list=ko.computed(function(){return settings.active_agents.filter(function(elem){return self.partner_agents().indexOf(String(elem.id))!==-1;});});self.case_agents=ko.computed(function(){var owner=self.owner_agent();if(_.pluck(self.partner_agent_list(),"id").indexOf(owner.id)===-1){var all_agents=self.partner_agent_list().concat(owner);all_agents.sort(function(a,b){if(a.last==b.last){return a.first.localeCompare(b.first);}
...
var cur_agent=settings.active_agents.filter(function(elem){return elem.id===agent_id;}).first();return cur_agent.first+" "+cur_agent.last;};self.get_agent_splits_for_product=function(product_id){return _.filter(self.selected_agent_splits(),function(split){return!!split.agent_id&&split.product_id===product_id;});};self.are_agent_splits_complete=function(product_id){if(!self.has_agent_splits()){return true;}
...
return start;},[]);default_agents=_.filter(default_agents,function(a_id){if(a_id===null)return true;return _.find(self.case_agents(),function(case_agent){return case_agent.id===a_id;});});if(!~default_agents.indexOf(null)){default_agents.push(null);}
