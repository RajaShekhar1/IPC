...
var CaseViewModel=function CaseViewModel(case_data,product_choices,can_edit_case,settings,product_rate_levels){var self=this;self.case_id=case_data.id;self.case_token=case_data.case_token;self.product_rate_levels=product_rate_levels||{};self.can_edit_case=can_edit_case;self.report_data=ko.observable(null);self.report_viewmodel=ko.observable(null);self.report_viewmodel.subscribe(function(val){if(val&&window.location.hash=="#reports"){val.load_reports();val.load_enrollments();}});self.company_name=ko.observable(case_data.company_name||"").extend({rateLimit:1000});self.group_number=ko.observable(case_data.group_number||"");self.product_choices=ko.observableArray(product_choices);var initially_selected_products=_.chain(self.product_choices()).filter(function(p){return _.includes(_.map(case_data.products,"id"),p.id);}).value();initially_selected_products=_.sortBy(initially_selected_products,function(product){return product.ordinal;});self.products=ko.observableArray(initially_selected_products);self.sort_selected_products=ko.observableArray([]);self.products.subscribe(function(){self.is_data_dirty(true);self.update_product_ordinals();});self.has_product_sort_selections=ko.pureComputed(function(){return!!self.sort_selected_products()&&self.sort_selected_products().length>0;});self.unselected_products=ko.pureComputed(function(){return _.sortBy(_.difference(self.product_choices(),self.products()),function(product){return product.ordinal;});});self.selected_product=ko.observable(null);self.has_selected_product=ko.pureComputed(function(){return!!self.selected_product();});self.add_selected_product=function(){var product=self.selected_product();if(!product){return;}
...
return true;});self.payment_mode_choices=settings.payment_modes;self.payment_mode=ko.observable(_.find(self.payment_mode_choices,function(c){return c.mode==case_data.payment_mode;}));self.selected_payment_mode=ko.pureComputed(function(){return(self.payment_mode())?parseInt(self.payment_mode().mode):null;});self.include_bank_draft_form=ko.observable(case_data.include_bank_draft_form);self.can_include_bank_draft_form=ko.pureComputed(function(){return self.selected_payment_mode()===12;});self.can_include_bank_draft_form.subscribe(function(can_include){if(!can_include){self.include_bank_draft_form(false);}});self.should_use_call_center_workflow=ko.observable(case_data.should_use_call_center_workflow);self.occupation_classes=ko.observableArray(_.map(case_data.occupation_class_settings,function(occupation){return new OccupationVM(occupation.label,occupation.level);}));self.new_occupation_class=ko.observable('');self.addOccupationClass=function(){self.occupation_classes.push(new OccupationVM(self.new_occupation_class()));self.new_occupation_class('');};self.removeOccupationClass=function(occupation_class){self.occupation_classes.remove(occupation_class);};self.is_new_occupation_class_valid=ko.pureComputed(function(){var index=_.findIndex(self.occupation_classes(),function(item){return item.label.toLowerCase()===self.new_occupation_class().toLowerCase();});return(self.new_occupation_class()!==''&&index===-1);});self.available_occupation_mappings=[{value:1,label:'1'},{value:2,label:'2'},{value:3,label:'3'}];self.hi_occupation_classes=[];self.eligible_product_codes=['ACC','HI'];self.is_product_occupation_class_eligible=function(product){return self.eligible_product_codes.indexOf(product.base_product_type)!=-1;};self.is_case_occupation_class_eligible=function(){return _.any(_.map(case_settings.products(),function(p){return self.is_product_occupation_class_eligible(p);}));};self.occ_mapping_cache={};self.update_occ_vm=function(product_id,label,value){var is_update=false;if(product_id in self.occ_mapping_cache){if(!(label in self.occ_mapping_cache[product_id])){is_update=true;}}else{self.occ_mapping_cache[product_id]={};self.occ_mapping_cache[product_id][label]={};is_update=true;}
...
return r;};self._case_rider_configurations_by_product={};self.case_riders=ko.pureComputed(function(){var _case_rider_configs=[];_.each(self.products(),function(product){if(!_.has(self._case_rider_configurations_by_product,product.id)){self._case_rider_configurations_by_product[product.id]=_.map(product.riders,function(r){return new CaseRiderConfiguration(self,product,r,case_data.product_settings);});}
...
function build_product(root,product_data){var base_type=product_data.base_product_type;var base_product;if(base_type==="FPPTI"||base_type==="FPPTIW"||base_type=="FPPTIB"||base_type=="FPPTIY"){base_product=new FPPTIProduct(product_data);}else if(base_type==="FPPCI"){base_product=new FPPCIProduct(product_data);}else if(base_type==="Group CI"){base_product=new GroupCIProduct(root,product_data);}else if(base_type==="FPP-Gov"){base_product=new FPPGovProduct(product_data);}else if(base_type==="ACC"){base_product=new ACCProduct(product_data);}else if(base_type==='HI'){base_product=new HIProduct(product_data);}else{alert("Invalid product type '"+base_type+"'");base_product=new FPPTIProduct(product_data);}
return base_product;}
function Product(){}
...
init_validation(self);self.get_simple_coverage_options=ko.pureComputed(function(){var options=[{label:self.employee().first(),coverage:new SimpleCoverageOption({premium:1.00,value:'EE'})}];if(self.should_include_spouse_in_table()){options.push({label:self.employee().first()+' + '+self.spouse().first(),coverage:new SimpleCoverageOption({premium:2.00,value:'ES'})});}
if(self.should_include_children_in_table()){options.push({label:self.employee().first()+' + Children',coverage:new SimpleCoverageOption({premium:1.50,value:'EC'})});}
if(self.should_include_spouse_in_table()&&self.should_include_children_in_table()){options.push({label:'All Family',coverage:new SimpleCoverageOption({premium:2.50,value:'EF'})});}
return options;});self.has_spouse=ko.computed(function(){return!!self.spouse();});self.has_children=ko.computed(function(){return!!self.children()&&self.children().length>0;});self.spouse_and_employee_names=ko.computed(function(){if(self.spouse()){return self.employee().first()+' + '+self.spouse().first();}else{return self.employee().first();}});}
...
return{refresh_census_table:refresh_census_table,refresh_enrollments_table:refresh_enrollments_table,init_alphabet_search:init_alphabet_search,init_status_filter:init_status_filter};})();var OccupationVM=(function(label,level,product_id){'use strict';var self=this;self.product_id=product_id||null;self.label=label;self.serialize_object=function(){return{product_id:self.product_id,label:self.label};};});function format_face_value(val){if(val==null){return"(NA)";}
