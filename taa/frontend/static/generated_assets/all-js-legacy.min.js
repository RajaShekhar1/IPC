...
self.restrict_download_enrollments_to_owner=ko.observable(should_restrict_downloads);self.can_partners_download_enrollments=ko.pureComputed(function(){return!self.restrict_download_enrollments_to_owner();});self.can_download_enrollments=ko.pureComputed(function(){return self.has_census_data();});self.partner_agents=ko.observable((case_data.partner_agents)?_.map(_.pluck(case_data.partner_agents,"id"),function(id){return id+"";}):[]);self.state_product_limiter=new ProductStatesLimiterViewModel(settings.product_state_mapping,self.situs_state,self.state_choices,self.products,self.product_choices);self.is_editing_products=ko.observable(false);if(case_data.products.length===0){self.is_editing_products(true);}
self.start_editing_products=function(){self.is_editing_products(true);};self.stop_editing_products=function(){self.is_editing_products(false);};self.state_product_override_limiter=new StatesLimiterViewModel(settings.product_state_mapping,self.enrollment_state_override,self.state_choices,self.products);self.is_open_enrollment=ko.computed(function(){return self.enrollment_period_type()==="open";});self.is_annual_enrollment=ko.computed(function(){return self.enrollment_period_type()==="annual";});self.get_open_enrollment_period=ko.computed(function(){return _.find(self.enrollment_periods(),function(p){return p.is_open();});});self.annual_enrollment_periods=ko.computed(function(){return _.filter(self.enrollment_periods(),function(p){return p.period_type==="annual_period";});});self.payment_mode_choices=settings.payment_modes;self.payment_mode=ko.observable(_.find(self.payment_mode_choices,function(c){return c.mode==case_data.payment_mode;}));self.selected_payment_mode=ko.pureComputed(function(){return(self.payment_mode())?parseInt(self.payment_mode().mode):null;});self.include_bank_draft_form=ko.observable(case_data.include_bank_draft_form);self.can_include_bank_draft_form=ko.pureComputed(function(){return self.selected_payment_mode()===12;});self.can_include_bank_draft_form.subscribe(function(can_include){if(!can_include){self.include_bank_draft_form(false);}});self.occupation_classes=ko.observableArray(case_data.occupation_class_settings);self.new_occupation_class=ko.observable('');self.addOccupationClass=function(){self.occupation_classes.push({label:self.new_occupation_class()});self.new_occupation_class('');};self.removeOccupationClass=function(occupation_class){self.occupation_classes.remove(occupation_class);};self.is_new_occupation_class_valid=ko.computed(function(){var index=_.findIndex(self.occupation_classes(),function(item){return item.label.toLowerCase()===self.new_occupation_class().toLowerCase();});return(self.new_occupation_class()!==''&&index===-1);});self._case_rider_configurations_by_product={};self.case_riders=ko.computed(function(){var _case_rider_configs=[];_.each(self.products(),function(product){if(!_.has(self._case_rider_configurations_by_product,product.id)){self._case_rider_configurations_by_product[product.id]=_.map(product.riders,function(r){return new CaseRiderConfiguration(self,product,r,case_data.product_settings);});}
...
self.owner_agent=ko.computed(function(){return settings.active_agents.filter(function(elem){return elem.id===parseInt(self.owner_agent_id(),10);});});self.partner_agent_list=ko.computed(function(){return settings.active_agents.filter(function(elem){return self.partner_agents().indexOf(String(elem.id))!==-1;});});self.case_agents=ko.computed(function(){return self.partner_agent_list().concat(self.owner_agent());});self.has_agent_splits=ko.observable(case_data.is_stp);self.selected_agent_splits=ko.observableArray(get_initial_splits());self.selected_agent=ko.observable();self.selected_agents=ko.observableArray(get_initial_split_agents());self.unused_agents=ko.computed(function(){var selected_ids=self.selected_agents();return self.case_agents().filter(function(i){return!~selected_ids.indexOf(i.id);});});self.add_agent_split=function(){var selected_agent=self.selected_agent();if(selected_agent){self.selected_agents.push(selected_agent.id);}};self.get_agent_name_from_id=function(agent_id){if(agent_id===null){return"Writing Agent";}
var cur_agent=settings.active_agents.filter(function(elem){return elem.id===agent_id;}).first();return cur_agent.first+" "+cur_agent.last;};function AgentSplit(agent_id,product_id,commission_subcount_code,split_percentage){this.agent_id=agent_id;this.product_id=product_id;this.commission_subcount_code=ko.observable(commission_subcount_code);this.split_percentage=ko.observable(split_percentage);this.valid=ko.computed(function(){if(this.agent_id===null){return true;}
...
return{Applicant:Applicant,ApplicantGroup:ApplicantGroup,ApplicantList:ApplicantList,create_applicant:create_applicant,age_for_date:age_for_date};})();var applicant_coverage_selection=(function(){function ApplicantCoverageSelection(){}
return{ApplicantCoverageSelection:ApplicantCoverageSelection};})();var beneficiary=(function(){function Beneficiary(options){var self=this;var defaults={name:"",relationship:"",ssn:"",date_of_birth:"",is_nonperson_entity:false};options=$.merge({},defaults,options);self.name=ko.observable(options.name);self.relationship=ko.observable(options.relationship);self.ssn=ko.observable(options.ssn);self.date_of_birth=ko.observable(options.date_of_birth);self.is_nonperson_entity=ko.observable(options.is_nonperson_entity);self.serialize=function(){return{name:self.name(),relationship:self.relationship(),ssn:self.ssn(),date_of_birth:self.date_of_birth(),is_nonperson_entity:self.is_nonperson_entity()};};}
return{Beneficiary:Beneficiary}})();function BenefitsPackage(root,name){var self=this;self.root=root;self.name=ko.observable(name);self.employee_recommendation=ko.observable(new NullRecommendation());self.spouse_recommendation=ko.observable(new NullRecommendation());self.children_recommendation=ko.observable(new NullRecommendation());self.set_recommendations=function(recommendations){if(root.employee().is_valid()){self.employee_recommendation(self.build_recommendation(self.root.employee(),recommendations['employee']));}else{self.employee_recommendation(new NullRecommendation());}
...
if(root.should_include_children_in_table()){self.children_recommendation(self.build_recommendation(self.root.child_benefits(),recommendations['children']));}else{self.children_recommendation(new NullRecommendation());}};self.build_recommendation=function(applicant,recommended_val){var benefit=self.get_recommended_benefit(applicant,recommended_val);return new Recommendation(benefit);};self.get_recommended_benefit=function(applicant,recommended_val){if(recommended_val==null||recommended_val==""){return new NullCoverageOption();}else{var applicant_type;if(applicant==root.employee()){applicant_type="employee";}else if(applicant==root.spouse()){applicant_type="spouse";}else{applicant_type="children";}
...
return benefits;};self.get_total_riders=ko.computed(function(){total_rider_amount=0;if(window.ui){var riders=window.ui.get_selected_riders();for(var i=0;i<riders.length;i++){total_rider_amount+=window.ui.riders()['emp'][riders[i].code];}}
return total_rider_amount;});self.get_total_premium=ko.computed(function(){var benefits=self.get_package_benefits();var total=0.0;$.each(benefits,function(){if(this.premium!=null){total+=this.premium;}});if(self.get_total_riders()>0.0){total+=self.get_total_riders();}
return total;});self.formatted_total_premium=ko.computed(function(){if(self.get_total_premium()>0.0){return format_premium_value(self.get_total_premium());}else{return"";}});self.is_valid=function(){return true};self.did_select_employee_coverage=ko.computed(function(){return self.employee_recommendation().recommended_benefit.is_valid();});self.did_select_spouse_coverage=ko.computed(function(){return self.spouse_recommendation().recommended_benefit.is_valid();});self.did_select_children_coverage=ko.computed(function(){return self.children_recommendation().recommended_benefit.is_valid();});self.get_all_people=ko.computed(function(){var employee=root.employee();var people=[employee];if(root.should_include_spouse_in_table()){var spouse=root.spouse();people.push(spouse);}
if(root.should_include_children_in_table()){$.each(root.get_valid_children(),function(){var child=this;people.push(child);});}
return people;});self.get_all_covered_people=ko.computed(function(){var people=[];return people;});self.get_all_people_labels=ko.computed(function(){var labels=[root.employee().name()];if(root.should_include_spouse_in_table()){labels.push(root.spouse().name());}
if(root.should_include_children_in_table()){$.each(root.get_valid_children(),function(){var child=this;labels.push(child.name());});}
return labels;});self.get_all_covered_people_labels=ko.computed(function(){var labels=[];return labels;});self.get_covered_applicants_with_type=ko.computed(function(){var applicants=[];if(root.did_select_employee_coverage()){applicants.push({applicant:root.employee(),type:"Employee",coverage:self.employee_recommendation().recommended_benefit});}
...
function NullCoverageOption(){var self=this;self.is_by_face=true;self.premium=0;self.face_value=0;self.is_valid=function(){return false;};self.format_premium_option=function(){return"";};self.format_premium=function(){};self.format_face_value=function(){return"- no benefit -";};self.format_for_dropdown=function(){return"- no benefit -";};self.serialize_data=function(){return{}};self.payment_mode=function(){return null;};}
...
return self.product_coverage.did_select_children_coverage();};self.can_employee_skip_due_to_GI=function(){return false};self.can_spouse_skip_due_to_GI=function(){return false};self.can_child_skip_due_to_GI=function(child){return false};self.show_yes_dialogue_employee=function(){return self.show_yes_dialogue();};self.show_yes_dialogue_spouse=function(){return self.show_yes_dialogue();};self.show_yes_dialogue_child=function(child){return self.show_yes_dialogue();};self.show_yes_dialogue=function(){if(self.does_yes_stop_app()){handle_question_yes();}else{}};self.does_any_applicant_need_to_answer=ko.pureComputed(function(){return _.any(self.product_coverage.applicant_coverage_selections(),function(app_cov){return self.does_applicant_need_to_answer(app_cov.applicant);});});};StandardHealthQuestion.prototype.is_spouse_only=function(){return this.question.is_spouse_only;};StandardHealthQuestion.prototype.get_question_text=function(){return this.question.question_text;};StandardHealthQuestion.prototype.get_question_label=function(){return this.question.label;};StandardHealthQuestion.prototype.does_applicant_need_to_answer=function(applicant){if(applicant.type===wizard_applicant.Applicant.EmployeeType){return this.does_employee_need_to_answer();}
if(applicant.type===wizard_applicant.Applicant.SpouseType){return this.does_spouse_need_to_answer();}
if(applicant.type===wizard_applicant.Applicant.ChildType){return this.does_child_need_to_answer(applicant);}
return false;};StandardHealthQuestion.prototype.can_applicant_skip_due_to_GI=function(applicant){if(applicant.type===wizard_applicant.Applicant.EmployeeType){return this.can_employee_skip_due_to_GI();}else if(applicant.type===wizard_applicant.Applicant.SpouseType){return this.can_spouse_skip_due_to_GI();}else if(applicant.type===wizard_applicant.Applicant.ChildType){return this.can_child_skip_due_to_GI(applicant);}else{console.error("Invalid applicant type "+applicant.type,applicant);}};StandardHealthQuestion.prototype.get_yes_highlight=function(){return(this.does_yes_stop_app())?'stop':'checkmark';};StandardHealthQuestion.prototype.does_yes_stop_app=function(){return!this.question.is_ignored;};var GIHealthQuestion=function(product,question,product_coverage,applicant_criteria,skip_mode,skipped_questions){var self=this;self.product=product;self.product_coverage=product_coverage;self.question=question;self.applicant_criteria=applicant_criteria;self.skip_mode=skip_mode;self.skipped_questions=skipped_questions;self.get_criteria=function(applicant_type){if(applicant_type=="Children"){applicant_type="Child";}
return _.filter(self.applicant_criteria,function(c){return c.applicant_type==applicant_type;});};self.get_met_gi_criteria_for_employee=ko.computed(function(){if(!self.product_coverage.did_select_employee_coverage()){return undefined;}
var applicant=self.product_coverage.applicant_list.get_employee();var coverage=self.product_coverage.__get_coverage_for_applicant(applicant).coverage_option();var employee_criteria=self.get_criteria("Employee");return _.find(employee_criteria,function(criterion){return self.does_applicant_meet_GI_criteria(applicant,coverage,criterion);});});self.has_employee_met_GI_criteria=ko.computed(function(){return self.get_met_gi_criteria_for_employee()!==undefined;});self.get_met_gi_criteria_for_spouse=ko.computed(function(){if(!self.product_coverage.did_select_spouse_coverage()){return undefined;}
var applicant=self.product_coverage.applicant_list.get_spouse();var coverage=self.product_coverage.__get_coverage_for_applicant(applicant).coverage_option();var criteria=self.get_criteria("Spouse");return _.find(criteria,function(criterion){return self.does_applicant_meet_GI_criteria(applicant,coverage,criterion);});});self.has_spouse_met_GI_criteria=ko.computed(function(){return self.get_met_gi_criteria_for_spouse()!==undefined;});self.can_employee_skip_due_to_GI=ko.computed(function(){return self.has_employee_met_GI_criteria()&&self.should_skip_if_GI_criteria_met();});self.can_spouse_skip_due_to_GI=ko.computed(function(){return self.has_spouse_met_GI_criteria()&&self.should_skip_if_GI_criteria_met();});self.get_met_gi_criteria_for_child=function(child_applicant){if(!self.product_coverage.did_select_children_coverage()){return undefined;}
var criteria=self.get_criteria("Child");var children=self.product_coverage.applicant_list.get_children_group();var coverage=self.product_coverage.__get_coverage_for_applicant(children).coverage_option();return _.find(criteria,function(criterion){return self.does_applicant_meet_GI_criteria(child_applicant,coverage,criterion);});};self.has_child_met_GI_criteria=function(child_applicant){return self.get_met_gi_criteria_for_child(child_applicant)!==undefined;};self.show_yes_dialogue_employee=function(){self.show_yes_dialogue('Employee',self.product_coverage.applicant_list.get_employee());};self.show_yes_dialogue_spouse=function(){self.show_yes_dialogue('Spouse',self.product_coverage.applicant_list.get_spouse());};self.show_yes_dialogue_child=function(child){self.show_yes_dialogue('Children',child);};self.get_best_GI_criteria=function(applicant,coverage,criteria){var descendingCriteria=_.sortBy(criteria,function(c){return-c.guarantee_issue_amount;});return _.find(descendingCriteria,function(criterion){return(self.does_applicant_meet_demographic_GI_criteria(applicant,criterion)&&criterion.guarantee_issue_amount<coverage.face_value);});};self._clean_criteria_applicant_type=function(applicant_type){if(applicant_type==='employee'){return'Employee';}else if(applicant_type==='spouse'){return'Spouse';}else if(applicant_type==='child'||applicant_type==='children'){return'Children';}else{return applicant_type;}};self.show_yes_dialogue=function(applicant_type,applicant){applicant_type=self._clean_criteria_applicant_type(applicant_type);if(!self.should_skip_if_GI_criteria_met()){handle_question_yes();return;}
var reduced_gi_criterion;var criteria=self.get_criteria(applicant_type);var coverage=self.product_coverage.__get_coverage_for_applicant(applicant).coverage_option();if(applicant_type=="Employee"){reduced_gi_criterion=self.get_best_GI_criteria(applicant,coverage,criteria);}else if(applicant_type=="Spouse"){reduced_gi_criterion=self.get_best_GI_criteria(applicant,coverage,criteria);}else if(applicant_type=="Children"){reduced_gi_criterion=self.get_best_GI_criteria(applicant,coverage,criteria);}
if(!reduced_gi_criterion){handle_question_yes();return;}
var face_amount=coverage.format_face_value();var gi_amount=reduced_gi_criterion.guarantee_issue_amount;var formatted_gi_amount=format_face_value(gi_amount);var button_options={reduce:{label:"Reduce the coverage",className:'btn-success',callback:function(){var applicant_coverage_options=self.product_coverage.get_coverage_options_for_applicant(applicant);var max_option=_.max(_.filter(applicant_coverage_options,function(o){return o.face_value<=gi_amount&&o.face_value>0}),function(o){return o.face_value});self.product_coverage.__get_coverage_for_applicant(applicant).customized_coverage_option(max_option);}},remove:{label:"Remove this applicant",className:'btn-danger',callback:function(){var applicant_coverage_options=self.product_coverage.get_coverage_options_for_applicant(applicant);if(applicant_type=="Children"){self.product_coverage.applicant_list.remove_applicant(applicant);}else{var null_option=_.find(applicant_coverage_options,function(o){return o.face_value==0});self.product_coverage.__get_coverage_for_applicant(applicant).customized_coverage_option(null_option);}}},ignore:{label:"Ignore and Continue",className:'btn-default',callback:function(){}}};bootbox.dialog({message:'A "yes" response to this question prohibits this person from obtaining the selected '+face_amount+' of coverage. You may proceed, however, by reducing your coverage to the guaranteed coverage amount of '+formatted_gi_amount+'.'+'<br><br>Alternatively, you may remove this individual from the coverage selection altogether (in Step 1) before proceeding with the rest of the application.',buttons:button_options});};self.does_employee_need_to_answer=ko.computed(function(){if(!self.product_coverage.did_select_employee_coverage()){return false;}
...
self.all_coverage_options.children(valid_children_options);var current_child_coverage=self.root.selected_plan().children_recommendation().recommended_benefit;if(!_.find(valid_children_options,function(opt){return opt.face_value==current_child_coverage.face_value;})){self.root.selected_plan().children_recommendation(new NullRecommendation(null_child_option));}};self.update_children_coverage_options=function(){var emp_benefit=self.root.selected_plan().employee_recommendation().recommended_benefit;};self.does_override_rate_options=function(applicant_type){return(applicant_type===wizard_applicant.Applicant.EmployeeType||applicant_type===wizard_applicant.Applicant.SpouseType);};self.get_coverage_options_for_applicant=function(applicant_type){return this.all_coverage_options[applicant_type];};function convert_rate_to_benefit_option(rate){return self.get_new_benefit_option({is_by_face:true,face_value:rate.coverage,premium:rate.premium});}
self.filter_coverage_options_for_applicant_type=function(all_options,applicant_type){if(applicant_type===wizard_applicant.Applicant.EmployeeType){return self.filter_base_rate_options(all_options);}else if(applicant_type===wizard_applicant.Applicant.SpouseType){var emp_coverage=window.vm.coverage_vm.get_applicant_coverage_for_product(window.vm.employee(),self);var options=self.filter_base_rate_options(all_options);var emp_benefit=emp_coverage.coverage_option();var valid_options=[];var anyYesQuestions=false;if(!emp_benefit.is_valid()||anyYesQuestions){return _.filter(options,function(o){return o.face_value<=25000;});}else{var limit=_.min([emp_benefit.face_value/2.0,25000]);options=_.filter(options,function(o){return o.face_value<=limit});if(options.length&&options[options.length-1].face_value<limit){var half_opt=_.find(all_options,function(o){return o.face_value===limit});if(half_opt){options.push(half_opt);}}
return options;}}
return all_options;};self.filter_base_rate_options=function(all_options){return _.filter(all_options,function(o){return o.face_value%5000===0;});};self.parse_benefit_options=function(applicant_type,applicant,rates){var self=this;if(applicant_type=="employee"){if(self.employee_current_benefit_subscription===null){self.employee_current_benefit_subscription=applicant.selected_plan.subscribe(self.update_spouse_coverage_options);var subscribe_to_question_answers=function(questions){_.each(questions,function(soh_answer){soh_answer.answer.subscribe(self.update_spouse_coverage_options);});};applicant.health_questions.subscribe(subscribe_to_question_answers);subscribe_to_question_answers(applicant.health_questions());}
var valid_options=[new NullCoverageOption()];var rate_choices=[];$.each(rates.byface,function(){var rate=this;if(rate.coverage%5000==0){rate_choices.push(rate);}});$.merge(valid_options,$.map(rate_choices,convert_rate_to_benefit_option));self.all_coverage_options[applicant_type](valid_options);}
...
if(rate.coverage<=25000){all_spouse_rates.push(rate);}});self.spouse_options_for_demographics($.merge([new NullCoverageOption()],$.map(demographic_spouse_rates,convert_rate_to_benefit_option)));self.all_spouse_rate_options($.merge([new NullCoverageOption()],$.map(all_spouse_rates,convert_rate_to_benefit_option)));var sp_options=[new NullCoverageOption()];$.merge(sp_options,$.map(demographic_spouse_rates,convert_rate_to_benefit_option));self.all_coverage_options.spouse(sp_options);}
if(applicant_type=="children"&&rates.byface!==undefined){var ch_options=[new NullCoverageOption()];$.merge(ch_options,$.map(rates.byface,convert_rate_to_benefit_option));self.all_posible_children_options(ch_options);self.all_coverage_options.children(ch_options);}};}
...
return{build_products:build_products};})();function RecommendationSet(name,recommendations){var self=this;self.name=name;self.recommendations=recommendations;self.get_recommended_applicant_coverage=function(applicant_type){var rec=_.find(self.recommendations,function(rec){return rec.applicant_type===applicant_type;});return rec.recommended_coverage;};self.formatted_payment_mode=function(){if(self.recommendations.length===0){return"";}
if(!_.any(_.invoke(self.recommendations,"is_valid"))){return"";}
var valid_recommendation=_.find(self.recommendations,function(rec){return rec.is_valid()});return valid_recommendation.recommended_coverage.payment_mode().display_lowercase();};self.formatted_total_premium=function(){return format_premium_value(self.get_total_premium());};self.get_total_premium=function(){var total=0.0;_.each(self.recommendations,function(rec){total+=rec.recommended_coverage.premium;});return total;};}
function Recommendation(name,applicant_type,coverage_option){var self=this;self.name=name;self.applicant_type=applicant_type;self.recommended_coverage=coverage_option;self.is_valid=function(){return self.recommended_coverage.is_valid();};self.format_premium_option=function(){return self.recommended_coverage.format_premium_option()};self.format_coverage=function(){return self.recommended_coverage.format_face_value();};}
function NullRecommendation(name){var self=this;self.name=name;self.recommended_coverage=new NullCoverageOption();self.is_valid=function(){return false;};self.format_premium_option=function(){return"";};self.format_coverage=function(){return"";};}
...
wizard_results['children']=[];wizard_results['child_coverages']=[];wizard_results['children_soh_questions']=[];_.each(product_cov.get_covered_children(),function(child){wizard_results['children'].push(child.serialize_data());var coverage=product_cov.__get_coverage_for_applicant(child);wizard_results['child_coverages'].push(coverage.coverage_option().serialize_data());if(health_questions){var soh_questions=health_questions.serialize_answers_for_applicant(child);wizard_results['children_soh_questions'].push(soh_questions);}});wizard_results['product_id']=product_cov.product.product_data.id;wizard_results.replacement_read_aloud=root.replacement_read_aloud();wizard_results.replacement_is_terminating=root.replacement_is_terminating();wizard_results.replacement_using_funds=root.replacement_using_funds();wizard_results.replacement_policies=_.invoke(root.replacement_policies(),"serialize");wizard_results.rider_data=product_cov.selected_riders.serialize_data();return wizard_results;}
function submit_decline(){var root=window.vm;var results=_.map(root.product_coverage_viewmodels(),function(product_cov){return build_wizard_results_for_product_coverage(product_cov);});ajax_post("/submit-wizard-data",{"wizard_results":results},function(resp){if(resp.error){bootbox.dialog({message:"There was a problem submitting the form ("+resp.error+").  Please contact the enrollment system administrator.",buttons:{"success":{"label":"OK","className":"btn-sm btn-primary"}}});}else{location=resp.redirect;}},handle_remote_error,true);}
...
if(info.step===1){$("#health_questions_error").html("");if(ui.did_decline_all_products()){submit_decline();e.preventDefault();return;}
var is_valid=ui.validator.form();if(!ui.is_coverage_selection_valid()){ui.show_no_selection_error();e.preventDefault();return;}
...
return true;}
_.each(ui.coverage_vm.selected_product_coverages(),function(product_coverage){_.each(product_coverage.applicant_coverage_selections(),function(applicant_coverage){var can_apply=validate_coverage_amount(applicant_coverage);if(!can_apply){is_valid=false;return false;}});if(!is_valid){return false;}});if(!is_valid){e.preventDefault();}
$(document.body).scrollTop(0);}
if(info.step==2&&info.direction=='next'){var is_valid=true;if(ui.did_select_any_fpp_product()&&(ui.should_show_replacement_form())){is_valid&=$('#questions-form').valid();}
...
if(info.step==5&&info.direction=='next'){if(!$('#step5-form').valid()){e.preventDefault();return;}
...
function WizardUI(defaults){var self=this;self.defaults=defaults;self.case_id=defaults.case_id;self.products_offerred=wizard_products.build_products(self,defaults.products);self.insurance_product=self.products_offerred[0];self.did_decline=ko.observable(false);self.selected_plan=ko.observable(new NullBenefitsPackage(self));var questions=process_spouse_question_data(self,defaults.spouse_questions,self.insurance_product.product_data);var soh_questions=process_health_question_data(self,defaults.health_questions,self.insurance_product.product_data);$.merge(questions,soh_questions);self.health_questions=ko.observableArray(questions);self.selected_recommendation=ko.observable(null);self.get_replacement_paragraphs=function(){var paragraph_map=self.insurance_product.get_replacement_paragraphs();var paragraphs=paragraph_map[self.root.enrollState];if(!paragraphs){return[];}
return paragraphs;};self.selected_benefits=ko.computed(function(){return[];});self.did_select_any_fpp_product=ko.pureComputed(function(){return _.find(self.product_selection,function(ps){return ps.insurance_product.is_fpp_product();});});self.is_in_person_application=ko.observable('is_in_person'in defaults&&defaults.is_in_person);self.is_self_enroll=ko.pureComputed(function(){return!self.is_in_person_application()});self.disclaimer_notice_confirmed=ko.observable(false);self.payroll_deductions_confirmed=ko.observable(false);self.identityToken=ko.observable("");self.identityType=ko.observable("");self.enrollCity=ko.observable(defaults.enroll_city||"");self.enrollState=defaults.state;self.was_state_provided=("state"in defaults&&defaults.state!==null&&defaults.state!="XX");self.company_name=ko.observable(defaults.company_name||"(Unknown Company)");self.existing_insurance=ko.observable(null);self.replacing_insurance=ko.observable(null);self.NAIC_AND_MI=['AK','AL','AR','AZ','CO','IA','LA','MD','ME','MS','MT','NC','NE','NH','NJ','NM','OH','OR','RI','SC','TX','UT','VA','VT','WI','WV','MI'];self.is_NAIC_OR_MI=function(){return self.did_select_any_fpp_product()&&_.contains(self.NAIC_AND_MI,self.enrollState);};self.is_KY_OR_KS=function(){return self.enrollState=="KY"||self.enrollState=="KS";};self.is_CT_DC_ND_VI=function(){return _.contains(['CT','DC','ND','VI'],self.enrollState);};self.is_non_NAIC_other=function(){return!(self.is_NAIC_OR_MI()||self.is_KY_OR_KS()||self.is_CT_DC_ND_VI());};self.get_has_existing_question_highlight=function(){if(self.is_self_enroll()){return'flag';}
...
self.selected_plan(new_plan);};self.should_rates_update=ko.computed(function(){var watch_data_values=[self.employee().birthdate,self.employee().is_smoker,self.employee().height,self.employee().weight,self.spouse().birthdate,self.spouse().is_smoker,self.spouse().weight,self.spouse().height,self.children,self.payment_mode,self.should_show_spouse,self.should_include_children_in_table];_.each(watch_data_values,function(observable){observable()});}).extend({rateLimit:{timeout:500,method:"notifyWhenChangesStop"},notify:'always'});self.should_rates_update.subscribe(self.update_rate_table);self.should_show_spouse.subscribe(function(val){if(self.should_show_spouse()){setTimeout(function(){self.validator.form();},0);}});self.riders=ko.observable({emp:[],sp:[]});self.case_riders=ko.observableArray();self.enrollment_riders=ko.observableArray();self.selected_riders={emp:ko.observableArray(),sp:ko.observableArray()};self.selected_riders.serialize_data=(function(){var selected=self.selected_riders;return ko.computed(function(){return{emp:selected.emp(),sp:selected.sp(),}});})();function get_rider_by_code(code){for(var i=0;i<self.enrollment_riders.length;i++){if(code==self.enrollment_riders[i].code){return self.enrollment_riders[i];}}}
self.current_person=ko.observable();self.get_selected_riders=ko.computed(function(){var person=self.current_person();if(!person){return[];}
if(person.type===wizard_applicant.Applicant.EmployeeType){return self.selected_riders['emp']();}else if(person.type===wizard_applicant.Applicant.SpouseType){return self.selected_riders['sp']();}
return[];});self.is_rider_checked=function(rider_code,prefix){if(!rider_code||!prefix){return;}
var rider=get_rider_by_code(rider_code);if(!rider){return;}
var per_person_riders=self.selected_riders[prefix]();var rider_index=per_person_riders.indexOf(rider);return rider_index===-1;};self.toggle_selected_riders=function(rider_code,prefix){if(!rider_code||!prefix){return;}
var rider=get_rider_by_code(rider_code);if(!rider){return;}
var per_person_riders=self.selected_riders[prefix]();var rider_index=per_person_riders.indexOf(rider);if(rider_index===-1){per_person_riders.push(rider);}else{per_person_riders.splice(rider_index,1);}
self.selected_riders[prefix](per_person_riders);};self.show_updated_rates=function(resp){var data=resp.data;self.insurance_product.parse_benefit_options('employee',self.employee(),data.employee_rates);self.insurance_product.parse_benefit_options('spouse',self.spouse(),data.spouse_rates);self.insurance_product.parse_benefit_options('children',self.child_benefits(),data.children_rates);if(data.recommendations){self.recommendations.good.set_recommendations(data.recommendations['good']);self.recommendations.better.set_recommendations(data.recommendations['better']);self.recommendations.best.set_recommendations(data.recommendations['best']);}
if(self.selected_plan().is_valid()){self.apply_selected_customization();}
self.is_rate_table_loading(false);};self.handle_update_rates_error=function(resp){if(resp.status===400&&resp.responseJSON&&resp.responseJSON.errors){$.each(resp.responseJSON.errors,function(){var field_name=this.field;var message=this.error;limit_error_lookup[field_name](true);self.validator.form();});}else{handle_remote_error();}};self.lose_input_focus=function(data,event){var element=$(event.target);element.blur();};self.did_select_employee_coverage=ko.computed(function(){});self.did_select_spouse_coverage=ko.computed(function(){});self.did_select_children_coverage=ko.computed(function(){});self.has_contingent_beneficiary_error=ko.computed(function(){});self.show_health_modal=function(){$("#health_modal").modal('show');};self.recommendations={good:new BenefitsPackage(self,'Good'),better:new BenefitsPackage(self,'Better'),best:new BenefitsPackage(self,'Best')};self.is_form_valid=ko.computed(function(){return(self.selected_plan().is_valid()&&self.selected_plan().has_at_least_one_benefit_selected());});self.attempted_advance_step=ko.observable(false);self.is_selection_error_visible=ko.computed(function(){return self.attempted_advance_step()&&!self.is_form_valid();});self.show_no_selection_error=function(){self.attempted_advance_step(true);};self.get_covered_applicants=ko.computed(function(){});self.get_all_applicants=ko.computed(function(){});self.do_any_health_questions_need_answering=ko.computed(function(){});self.did_decline.subscribe(function(val){if(val){bootbox.dialog({message:"Please note that even after declining enrollment, you may still change your decision and elect to enroll at a later time, provided that your employer's enrollment period is still active.",buttons:{"success":{"label":"Close","className":"btn-sm btn-primary"}}});}});self.can_decline=ko.computed(function(){return(!_.any(self.get_all_applicants(),function(applicant){return applicant.get_existing_coverage_amount_for_product(self.insurance_product.product_data.id);}));});self.is_employee_actively_at_work=ko.observable(null);self.has_spouse_been_treated_6_months=ko.pureComputed(function(){return _find_answer_for_spouse_question_with_label("Spouse Treated 6 Months");});self.has_spouse_been_disabled_6_months=ko.pureComputed(function(){return _find_answer_for_spouse_question_with_label("Spouse Disabled 6 Months");});function _find_answer_for_spouse_question_with_label(lbl){if(!self.did_select_spouse_coverage()){return null;}
var spouse_question=_.find(self.health_questions(),function(q){return q.get_question_label()===lbl;});if(!spouse_question){return null;}
if(spouse_question.can_spouse_skip_due_to_GI()){return"GI";}
var spouse_answer=_.find(self.spouse().health_questions(),function(answer){return answer.question.label===spouse_question.get_question_label();});if(!spouse_answer){return null;}
return spouse_answer.answer();}
self.exit_application=function(){bootbox.dialog({message:"Are you sure you want to exit? All data on this application will be discarded.",buttons:{default:{label:"Cancel",className:"btn-default",callback:function(){}},danger:{label:"Exit application and discard data",className:"btn-danger",callback:function(){window.location.href="/enrollment-case/"+self.case_id+"#enrollment";}}}});};}
...
if(this.applicants.has_valid_children()&&this.should_include_children()){_.each(this.applicants.get_children(),function(c){applicants_in_table.push(c);});}
...
ApplicantCoverageSelectionVM.prototype={select_recommended_coverage:function(recommendation_set){var coverage_option=recommendation_set.get_recommended_applicant_coverage(this.applicant.type);if(coverage_option){this.recommended_coverage_option(coverage_option);this.customized_coverage_option(null);}},select_custom_coverage:function(option){this.customized_coverage_option(option);},has_selected_coverage:function(){return this.coverage_option().is_valid();},get_selected_coverage:function(){return this.coverage_option();},format_name:function(){return this.applicant.name();},_get_coverage_options:function(){var options=[new NullCoverageOption()];options=$.merge(options,product_rates_service.get_product_coverage_options_for_applicant(this.product,this.applicant)());return options;},format_recommendation_coverage:function(recommendation_set){var coverage=recommendation_set.get_recommended_applicant_coverage(this.applicant.type);return coverage.format_face_value();},format_recommendation_premium:function(recommendation_set){var coverage=recommendation_set.get_recommended_applicant_coverage(this.applicant.type);return coverage.format_premium_option();},format_selected_coverage:function(){return this.get_selected_coverage().format_face_value();},format_selected_premium:function(){return this.get_selected_coverage().format_premium_option();}};function ApplicantRecommendationVM(applicant,recommendation_set){var self=this;self.applicant=applicant;self.recommended_coverage=recommendation_set.get_recommended_applicant_coverage(self.applicant.type);self.total_premium=recommendation_set.get_total_premium;self.is_valid=function(){return self.recommended_coverage.is_valid();};self.format_premium_option=function(){return self.recommended_coverage.format_premium_option();};self.format_coverage=function(){return self.recommended_coverage.format_face_value();};}
...
function handle_remote_error(request,retry_callback){if(request.status==401){if(ui.account_href!=null){prompt_login(retry_callback);}else{login_reauth(null,null,retry_callback);}}
else{alert("Sorry, an error occurred communicating with the server.");}}
...
ko.components.register('limited-state-select',{viewModel:function(params){this.limiter=params.limiter;this.is_disabled=ko.observable(params.disabled||false);},template:'<select name="enrollmentState" id="enrollmentState" data-bind="\
  ...
  disable: is_disabled\
  "> \
  </select>'});var ProductStatesLimiterViewModel=function(product_statecode_mapping,selected_state,available_states,selected_products,available_products){var self=this;self.product_state_mapping=map_states_to_products_from_statecode_map(available_states,product_statecode_mapping);self.selected_state=selected_state;self.available_states=available_states;self.selected_products=selected_products;self.available_products=available_products;self.is_valid_product_for_state=function(product,state){return _.contains(self.product_state_mapping[product.id],state);};self.is_state_disabled=function(state){return!_.contains(self.enabled_states(),state);};self.is_product_disabled=function(product){return!_.contains(self.enabled_products(),product);};self.disable_product_option_if_invalid=function(option,product){if(product===undefined){return;}
