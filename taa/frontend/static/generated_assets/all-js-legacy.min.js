...
function handle_enrollment_upload_error(err,modal){if(err.status>=400&&err.status<500){var errors=$.parseJSON(err.responseText);window.enrollment_api_panel.error_records(errors);show_error_panel($(modal));}else{alert("Sorry, there was a problem communicating with the server.");reset_upload_modal($(modal));}}
...
form_data.append('csv-file',files[0],files[0].name);form_data.append('upload_type',$("input[name=upload_type]:checked").val());send_file_data("POST",urls.get_submit_enrollment_records_url(),form_data,function(data){console.log(data);},function(err){handle_enrollment_upload_error(err,form);},false);show_loading_panel($(this));return false;});}
