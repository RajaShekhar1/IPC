...
return data;};self.is_product_group_ci=function(product){if(product.code=='Group CI'){self.initialize_states_selection(product);}
return product.code=='Group CI';};self.get_product_group_ci=function(){var group_ci=_.find(self.products(),function(product){return product.code=='Group CI';});if(typeof group_ci===typeof undefined){return false;}
return group_ci.id;};self.get_overridden_states=function(){return self.state_overrides();};self.get_product_id=function(product){return product.id;};self.initialize_states_selection=function(product){if(product.code=='Group CI'){$('#states-list-product-'+self.get_product_id(product)).bootstrapDualListbox({infoTextFiltered:'<span class="label label-purple label-lg">Filtered</span>',showFilterInputs:true,moveOnSelect:false,nonSelectedListLabel:'Available States:',selectedListLabel:'States Allowed:',selectorMinimalHeight:250});}};self.restrict_state_availability=ko.observable(false);self.state_overrides=ko.observableArray([]);self.initialize_state_overrides=function(){var group_ci=self.get_product_group_ci();var states=[];if(group_ci){if(!!case_data.product_settings){states=_.get(case_data.product_settings.state_overrides,String(group_ci));}}
if(states.length>0){self.restrict_state_availability(true);}
self.state_overrides(states);};self.initialize_state_overrides();self.occupation_classes_for_product=function(product){var occupation_mappings=[];if(!(product.id in self.occ_mapping_cache)){for(var i in self.occupation_classes()){if(i=='first'){continue;}
...
function build_product(root,product_data){var base_type=product_data.base_product_type;var base_product;if(base_type==="FPPTI"||base_type==="FPPTIW"||base_type=="FPPTIB"||base_type=="FPPTIY"){base_product=new FPPTIProduct(product_data);}else if(base_type==="FPPCI"){base_product=new FPPCIProduct(product_data);}else if(base_type==="Group CI"){base_product=new GroupCIProduct(root,product_data);}else if(base_type==="FPP-Gov"){base_product=new FPPGovProduct(product_data);}else if(base_type==="ACC"){base_product=new ACCProduct(product_data);}else if(base_type==='HI'){base_product=new HIProduct(product_data);}else if(base_type==='Static Benefit'){base_product=new MembershipProduct(product_data);}else{alert("Invalid product type '"+base_type+"'");base_product=new FPPTIProduct(product_data);}
return base_product;}
...
return hi_coverage_tiers;};HIProduct.prototype.create_coverage_option=function(options){return new SimpleCoverageOption(options);};HIProduct.prototype.is_fpp_product=function(){return false;};HIProduct.prototype.should_show_step_two=function(){return false;};HIProduct.prototype.should_show_step_four=function(){return false;};HIProduct.prototype.should_show_step_5=function(){return false;};function ACCProduct(product_data){Product.call(this);Object.defineProperty(this,'coverage_type',{value:CoverageType.Simple,configurable:true});this.product_type="ACC";this.product_data=product_data;}
...
wizard_results.children=[];_.each(product_cov.get_covered_children(),function(child){wizard_results['children'].push(child.serialize_data());});if(did_decline){return wizard_results;}
...
ProductCoverageViewModel.prototype={_get_effective_date:function(root){if(_.get(this.effective_date_settings,'effective_date_override')){return normalize_date(_.get(this.effective_date_settings,'effective_date'))}
...
var enroller_selects=self.which_enroller_select_date();if(!valid_enroller_selects(parseInt(enroller_selects.enroller_selects.no_less,10),self.get_effective_date())){results+=" The effective date input must be at least "+enroller_selects.enroller_selects.no_less+" days from today.";}
...
function view_envelope(envelope_id,options){var url="/envelopes/"+envelope_id+"/sign";if(options&&options.from_inbox){url+="?from=inbox";}
var req=$.post(url).success(function(data){return handle_signing_redirect(envelope_id,data)}).error(handle_signing_failure);bootbox.alert("Loading document, please wait...");return req;}
