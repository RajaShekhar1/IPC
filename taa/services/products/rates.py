...
class ProductRates(object):
    ...
    def get_all_rates(self, **demographics):
        ...
        if demographics.get('employee_age'):
            ...
            rates['employee'] = {
                'weekly_bypremium':
                    self.rate_table.get_coverages_by_weekly_premium(age),
                'weekly_byface':
                    self.rate_table.get_weekly_premiums_by_coverage(age),
                }
        ...
        if not emp_limits_table.is_height_weight_in_range(
            ...
            limit_errors += [
                dict(field='employee_height',
                     error='This height/weight combination is outside the range for this product.'),
...
class HeightWeightRangeTable(object):
    ...
    def is_height_weight_in_range(self, height, weight):
        ...
        return int(row['Min Weight']) <= int(weight) <= int(row['Max_Weight'])


GroupCI_Male_HeightWeightRangeTable = HeightWeightRangeTable(
    'taa/services/products/data_files/CIEMP-Male-Height-Weight-ranges.csv')
...
def build_GroupCI_smoking_rate_table():
    return CoverageByAgePremiumLookup(GROUP_CI_smoking_all_weekly_coverage_options,
                                      GROUP_CI_smoking_weekly_by_coverage_lookup)
