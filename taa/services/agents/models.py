...
class Agent(UserMixin, AgentJsonSerializable, db.Model):
    ...
    # Restrict only to states where agent is licensed
    # is_restricted_to_licensed_states = db.Column(db.Boolean,
    #                                              server_default='FALSE',
    #                                              nullable=False)
    # licensed_states = db.Column(JSON(none_as_null=False), nullable=True)
    
    # Other misc data transferred from stormpath
